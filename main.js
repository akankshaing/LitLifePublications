...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- new book launch popup ---------- */
  const launchOverlay = document.getElementById('launchPopupOverlay');
  if (launchOverlay) {
    const closeBtn = document.getElementById('launchPopupClose');
    const openPopup = () => launchOverlay.classList.add('show');
    const closePopup = () => launchOverlay.classList.remove('show');

    if (!sessionStorage.getItem('litlife_launch_popup_seen')) {
      setTimeout(openPopup, 900);
      sessionStorage.setItem('litlife_launch_popup_seen', '1');
    }
    if (closeBtn) closeBtn.addEventListener('click', closePopup);
    launchOverlay.addEventListener('click', (e) => { if (e.target === launchOverlay) closePopup(); });
